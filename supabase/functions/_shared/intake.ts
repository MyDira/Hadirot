...
7. EXACT ADDRESSES — when the listing gives a specific street address (a house number followed by a street name: "1438 53rd Street", "5012 14th Ave", "1279 E 8 St Apt 2"), put it in street_address as "<number> <full street name>" (expand the street name the same way as rule 6, e.g. "5012 14th Avenue"). Put any apartment/unit designation ("Apt 2", "#3R", "Unit B") in unit_number WITHOUT the "Apt"/"Unit"/"#" prefix. Rules:
   ...
   - If the text gives ONLY an intersection, street_address MUST be null. NEVER build an address out of cross streets, and never invent a house number.
...
11. For RENTALS put the monthly rent in "price" and set asking_price to null. For SALES put the asking price in "asking_price" and set price to null. If no price is given, set both to null, call_for_price=true, and price_note="call for price".
...
25. NEVER invent data. Missing value => null (or false for booleans). Accuracy matters far more than completeness — it is better to leave a field null than to guess.`;
...
export async function parseContent(
  ...
  return parsed.listings.map(normalizeParsedListing);
}

/**
 * Post-parse defaults the model must NOT guess at itself.
 *
 ...
 */
export function normalizeParsedListing(listing: ParsedListing): ParsedListing {
  const bathrooms =
    listing.listing_kind === 'rental' && (listing.bathrooms == null || listing.bathrooms <= 0)
      ? 1
      : listing.bathrooms;
