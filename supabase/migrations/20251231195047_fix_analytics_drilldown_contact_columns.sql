/*
  # Fix Analytics Listing Drilldown - Contact Submission Columns
  
  ## Issue
  The function references lcs.name which doesn't exist in listing_contact_submissions.
  Need to use the correct column names from the table.
*/
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      (SELECT jsonb_agg(jsonb_build_object('id', id, 'name', user_name, 'phone', user_phone, 'created_at', created_at) ORDER BY created_at DESC) FROM inquiry_details),
      '[]'::jsonb
    ) as inquiries
  FROM listings l
