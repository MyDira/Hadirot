/*
  # Fix Remaining Analytics Hub RPC Functions
  
  ## Issues Fixed
  1. analytics_listings_performance - ambiguous column reference (listing_id)
  2. analytics_zero_inquiry_listings - ensure consistent column usage
  3. analytics_listing_drilldown - ensure consistent column usage
  
  All other inquiry functions don't use analytics_events/analytics_sessions
  so they don't need fixes.
*/

-- Fix analytics_listings_performance (rename CTE column to avoid conflict)
...
-- Fix analytics_zero_inquiry_listings (ensure lcs prefix)
...
-- Fix analytics_listing_drilldown (ensure lcs prefix)
