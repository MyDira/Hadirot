/*
  # Fix Agency Slugify and Ensure Functions
  ...
  1. Database Functions
    ...
    - Ensure insert trigger hygiene for `public.set_agency_owner_default`

  2. Security
    - Add admin verification for cross-user agency operations
    - Maintain SECURITY DEFINER for RLS bypass
    - Safe slugification without encoding dependencies

  3. Legacy Claim Logic
    - First try to claim by `slugify(profile.agency)` if present
    - Then try to claim by `slugify(profile.full_name || 's Agency')`
    - Only create new agency if neither exists
*/
...
BEGIN
    ...
    -- Legacy claim expansion: First try to claim by profile.agency if present
    ...
    -- Then try to claim by default naming convention
...
-- Ensure insert trigger function is clean (no SECURITY DEFINER, only auth.uid())
...
-- Ensure the trigger exists (this is idempotent)
DO $$
BEGIN
    IF NOT EXISTS (
