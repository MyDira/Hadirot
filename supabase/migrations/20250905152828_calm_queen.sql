/*
  ...
  1. Drop and recreate analytics_summary function
  2. Fix COALESCE type mismatch by casting all keys to TEXT
  3. Use proper SQL language syntax with single SELECT
  4. Maintain NY timezone filtering and DISTINCT pre-aggregation
*/
...
events_in_range AS (
  SELECT
    e.id,
    e.event_name,
    /* normalize to TEXT only; do NOT reference a non-existent e.attempt_id */
    coalesce(
