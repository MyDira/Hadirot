...
> **⚠️ STATUS (SMS audit, July 8 2026): STILL OPEN.** The SMS audit reworked `handle-renewal-sms-webhook` (bulk-batch replies, commit `39b123d`) but did **not** add signature validation. It also introduced a **new** unauthenticated Twilio endpoint `sms-status-webhook` (`verify_jwt=false`, no signature check) with the same gap. Apply this fix to **both** functions.
...
> **🟡 STATUS (SMS audit, July 8 2026): PARTIALLY FIXED.** `send-listing-contact-sms` now rate-limits (3/hr per listing + duplicate-submit guard), derives the IP server-side, and accepts 11-digit phones (commit `902e2e8`). **`send-report-rented-sms` is still unauthenticated and unthrottled** — remaining half of this finding is open.
...
> **🟡 STATUS (SMS audit, July 8 2026): PARTIALLY IMPROVED.** `sms-status-webhook` was added to `config.toml` with `verify_jwt=false` (commit `2b88525`), so both Twilio webhooks are now declared. Still incomplete — most of the 42 functions remain undeclared.
