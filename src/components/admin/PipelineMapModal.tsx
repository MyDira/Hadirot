...
interface PipelineMapModalProps {
  listing: ScrapedListing | null;
  onClose: () => void;
}

export function PipelineMapModal({ listing, onClose }: PipelineMapModalProps) {
  const mapContainer = useRef<HTMLDivElement>(null);
  const map = useRef<mapboxgl.Map | null>(null);
  const [isLoaded, setIsLoaded] = useState(false);

  const hasCoords = listing && listing.latitude != null && listing.longitude != null && listing.geocode_status !== 'failed';
  ...
  useEffect(() => {
    ...
    const popupHtml = `
      <div style="font-family: system-ui, sans-serif; max-width: 220px;">
        <p style="font-weight: 600; margin: 0 0 4px;">${listing.title || crossStreets || 'Scraped listing'}</p>
        ${crossStreets ? `<p style="font-size: 13px; color: #555; margin: 0 0 4px;">${crossStreets}</p>` : ''}
        <span style="display: inline-block; font-size: 11px; padding: 2px 6px; border-radius: 4px; background: ${listing.geocode_status === 'success' ? '#dcfce7' : '#fef9c3'}; color: ${listing.geocode_status === 'success' ? '#166534' : '#854d0e'};">
          ${listing.geocode_status ?? 'unknown'}
        </span>
      </div>
    `;
        ...
        <div className="relative z-10 w-full max-w-2xl bg-white rounded-lg shadow-xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900">
              {[listing.cross_street_1, listing.cross_street_2].filter(Boolean).join(' & ') || 'Location'}
            </h3>
          ...
          {hasCoords ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-gray-700 font-medium mb-2">Geocoding failed for this listing</p>
