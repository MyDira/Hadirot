...
export function BrowseListings() {
  ...
  const NUM_STANDARD_SLOTS_PER_PAGE =
    ITEMS_PER_PAGE - NUM_FEATURED_INJECTED_SLOTS; // 16
  ...
  useEffect(() => {
    ...
    // Derive whoListing from legacy params
    if (poster_type === "landlord") {
      urlFilters.whoListing = "owner";
    } else if (poster_type === "agency") {
  ...
  const loadListings = async () => {
    try {
      ...
      console.log(
        "⭐ All featured listings available:",
      ...
      const startIndex =
        ((currentPage - 1) * featuredSlotsPerPage) % allFeaturedListings.length;
      let featuredForThisPage: Listing[] = [];

      if (allFeaturedListings.length > 0) {
        for (
          let i = 0;
          i < featuredSlotsPerPage && allFeaturedListings.length > 0;
          i++
        ) {
      ...
      console.log(
        "🎯 Featured injected for page",
        currentPage + ":",
        featuredForThisPage.map((l) => ({ id: l.id, title: l.title })),
      );
      ...
      console.log(
        "🗺️ Injected featured map:",
        Array.from(injectedFeaturedMap.keys()),
      );
      ...
      for (let i = 0; i < ITEMS_PER_PAGE; i++) {
        // Fill up to ITEMS_PER_PAGE slots
        ...
        if (
          ...
          !finalListings.some(
            (l) =>
              l.id === featuredForThisPage[featuredIndex].id &&
              l.showFeaturedBadge,
          )
        ) {
          ...
          const key = isAlsoInjected
            ? `${standardListing.id}-natural`
            : standardListing.id;
      ...
      console.log(
        "🎯 Final display listings for page",
        currentPage + ":",
        finalListings.map((l, i) => ({
          slot: i + 1,
          key: l.key,
          id: l.id,
          title: l.title,
          ...
          <div className="flex space-x-1">
            ...
            {(() => {
              ...
              for (let pageNum = startPage; pageNum <= endPage; pageNum++) {
                ...
                if (pageNum === totalPages && currentPage < totalPages)
                  continue;
